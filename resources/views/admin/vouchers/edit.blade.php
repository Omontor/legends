    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.vouchers.update", [$voucher->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('guid') ? 'is-invalid' : '' }}" type="text" name="guid" id="guid" value="{{ old('guid', $voucher->guid) }}" required>
                @if($errors->has('guid'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('en_description') ? 'is-invalid' : '' }}" type="text" name="en_description" id="en_description" value="{{ old('en_description', $voucher->en_description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('es_description') ? 'is-invalid' : '' }}" type="text" name="es_description" id="es_description" value="{{ old('es_description', $voucher->es_description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nl_description') ? 'is-invalid' : '' }}" type="text" name="nl_description" id="nl_description" value="{{ old('nl_description', $voucher->nl_description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fr_description') ? 'is-invalid' : '' }}" type="text" name="fr_description" id="fr_description" value="{{ old('fr_description', $voucher->fr_description) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('is_multi_use') ? 'is-invalid' : '' }}" name="is_multi_use" id="is_multi_use" required>
                    ...
                    @foreach(App\Models\Voucher::IS_MULTI_USE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_multi_use', $voucher->is_multi_use) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('is_for_group') ? 'is-invalid' : '' }}" name="is_for_group" id="is_for_group">
                    ...
                    @foreach(App\Models\Voucher::IS_FOR_GROUP_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_for_group', $voucher->is_for_group) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('group_size') ? 'is-invalid' : '' }}" type="number" name="group_size" id="group_size" value="{{ old('group_size', $voucher->group_size) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('commission_type') ? 'is-invalid' : '' }}" name="commission_type_id" id="commission_type_id" required>
                    @foreach($commission_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('commission_type_id') ? old('commission_type_id') : $voucher->commission_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
